...
import { fetchAllPlayers, fetchWeeklyPlayerStats, fetchNFLState } from '@/lib/sleeper-api'; // Add fetchNFLState for accurate week
...
function StatCard({ title, value }: { title: string, value: string | number }) {
    return (
        <div className="flex flex-col items-center gap-1 p-2 rounded-lg bg-background/50 flex-1 min-w-[80px]">
...
if (!player || !league || !player.gameLog || player.gameLog.length === 0) {
  return (
    <div className="text-center py-12">
      <p className="text-lg text-muted-foreground">No stats yet this season</p>
      <p className="text-sm text-muted-foreground mt-2">
        Check back after their first game!
      </p>
    </div>
  );
}
            ...
            <Table>
                ...
                <TableBody>
                    <TableRow>
                        ...
                        <TableCell className="text-right">{(player.weeklyProjection.passingTds || 0) + (player.weeklyProjection.rushingTds || 0) + (player.weeklyProjection.receivingTds || 0)}</TableCell>
                        <TableCell className="text-right">{player.weeklyProjection.fpts.toFixed(1)}</TableCell>
                    </TableRow>
                    {gameForProjectionWeek && (
                        <TableRow>
                        ...
                        {player.gameLog?.map((game) => (
                            <TableRow key={game.week}>
                                ...
                                <TableCell className="text-right">
                                 {(game.passingTds || 0) + (game.rushingTds || 0) + (game.receivingTds || 0) || '-'}
                                </TableCell>
                            </TableRow>
                        ))}
                    </TableBody>
...
function TeamView({ player }: { player: Player | null }) {
    ...
    const DepthChartView = ({ player }: { player: Player | null }) => {
        if (!player) return null;
       const teamPlayers = availablePlayers;
...
function StatsView({ player, league }: { player: Player | null; league: League | undefined }) {
  if (!player || !league) {
    return <p className="text-center py-8 text-muted-foreground">Player or league not found.</p>;
  }
  ...
  const isOffensivePlayer = offensivePositions.includes(player.position);
  const isDefensivePlayer = defensivePositions.includes(player.position);
  ...
  const getVisibleCategories = () => {
    const categories = Object.keys(statCategories) as (keyof typeof statCategories)[];
    if (isOffensivePlayer) {
      return categories.filter(cat => cat !== 'Defense' && cat !== 'Defensive Player');
    }
    if (isDefensivePlayer) {
      return categories.filter(cat => cat !== 'Passing' && cat !== 'Rushing' && cat !== 'Receiving' && cat !== 'Kicking');
    }
    return categories;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Live Current Week Section */}
      {currentWeekLiveStats && (
        <div className="mb-12 p-6 border-2 border-primary/30 rounded-xl bg-primary/5">
          <h4 className="text-2xl font-bold mb-6 text-primary">
            Current Week {player.gameLog[0]?.week || '??'} — Live Stats {livePoints > 0 && '(Updating Now!)'}
          </h4>
          {livePoints > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
              <div className="text-center">
                <div className="text-3xl font-bold text-primary">{livePoints.toFixed(2)}</div>
                <div className="text-sm text-muted-foreground">Live Fantasy Points</div>
              </div>
              {player.position === 'QB' && (
                <>
                  <StatCard title="Pass Yds" value={liveGameEntry?.passingYards ?? 0} />
                  <StatCard title="Pass TDs" value={liveGameEntry?.passingTds ?? 0} />
                  <StatCard title="INTs" value={liveGameEntry?.interceptions ?? 0} />
                </>
              )}
              {(player.position === 'RB' || player.position === 'WR' || player.position === 'TE') && (
                <>
                  <StatCard title="Rush Yds" value={liveGameEntry?.rushingYards ?? 0} />
                  <StatCard title="Rush TDs" value={liveGameEntry?.rushingTds ?? 0} />
                  <StatCard title="Rec Yds" value={liveGameEntry?.receivingYards ?? 0} />
                  <StatCard title="Rec TDs" value={liveGameEntry?.receivingTds ?? 0} />
                </>
              )}
              {player.position === 'K' && (
                <>
                  <StatCard title="FGM" value={liveGameEntry?.fgm ?? 0} />
                  <StatCard title="XPM" value={liveGameEntry?.xpm ?? 0} />
                </>
              )}
            </div>
          ) : (
            <p className="text-muted-foreground italic">
              Game hasn't started yet — live stats will appear here during/after the game!
            </p>
          )}
        </div>
      )}
      ...
      {/* Weekly Breakdown */}
      {player.gameLog.map((game) => (
        <div key={game.week} className="space-y-4 py-4 border-b last:border-0">
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              <TableRow>
                ...
                <TableCell>{game.opponent || 'BYE'}</TableCell>
                <TableCell className="text-right font-bold text-primary">
                  {game.fpts !== undefined ? calculateFantasyPoints(player, game, league.scoringSettings).toFixed(2) : '-'}
                </TableCell>
              </TableRow>
            </TableBody>
          </Table>
          ...
          <div className="px-4">
            {visibleCategories.map((category) => {
              const stats = statCategories[category];
...
export default function PlayerPage() {
  ...
  const { leagues, availablePlayers } = useContext(RosterContext);
  const { toast } = useToast();

  const league = useMemo(() => leagues[0] || null, [leagues]);
  // NEW: Load real player and full season game log from Sleeper
const [player, setPlayer] = useState<Player | null>(null);
const [isLoading, setIsLoading] = useState(true);
const [currentWeekLiveStats, setCurrentWeekLiveStats] = useState<any>(null);
  ...
  const loadPlayerAndStats = async () => {
    ...
    try {
      ...
      if (!localPlayer) {
        leagues.forEach(l => {
          l.teams?.forEach(t => {
            const found = [...t.roster.starters, ...t.roster.bench, ...t.roster.ir].find(p => p?.id === playerId);
...
useEffect(() => {
  ...
  const pollStats = async () => {
    ...
    const currentWeek = await getCurrentNFLWeek(); // Or use nflState
    const stats = await fetchWeeklyPlayerStats(year, currentWeek);
    setCurrentWeekLiveStats(stats[player.sleeperId]);
  };

  pollStats(); // Initial
  const interval = setInterval(pollStats, 30000); // Every 30s

  return () => clearInterval(interval);
}, [player?.sleeperId, league]);

  if (isLoading) {
    return <div className="p-6 text-center">Loading real player from Sleeper...</div>;
  }
            ...
            <Card>
                <CardHeader className="flex flex-row items-start gap-6">
                    <div className="flex-1 space-y-4">
                        <div>
                            <CardTitle className="text-3xl font-headline">{player.name}</CardTitle>
                            <CardDescription className="flex items-center gap-2 text-base text-muted-foreground mt-2">
                               <TeamHelmet team={player.nflTeam} className="w-6 h-6" />
                        ...
                        <Button variant="destructive" size="sm" onClick={handleDrop}>Drop Player</Button>
                    ...
                    alt={player.name}
                    className="w-24 h-24 sm:w-32 sm:h-32 rounded-full border-4 border-muted object-cover"
                    onError={(e) => {
                        e.currentTarget.src = 'https://picsum.photos/seed/player-fallback/128/128';
                    }}
                    />
                </CardHeader>
            </Card>
             ...
             <Card>
                <CardHeader>
                    <CardTitle>Stats & Projections</CardTitle>
                </CardHeader>
                <CardContent>
                    <Tabs defaultValue="Overview">
                        ...
                        <TabsContent value="Depth Chart" className="mt-4">
                            <p>Content for Depth Chart will go here.</p>
                        </TabsContent>
                    </Tabs>
                </CardContent>
            </Card>
